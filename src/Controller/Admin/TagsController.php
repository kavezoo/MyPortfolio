...
class TagsController extends AppController
{
    ...
    public function index()
    {
        ...
        if (isset($queryParams['clear']) && $queryParams['clear'] === 'filter') {
            $this->session->delete('Paging.Tags.params');
        ...
        // =========================================================================
        $searchableFields = [
            // --- 1. Saját tábla (Tags) mezői (sütéskor: string/uuid oszlopok) ---
            // 'Tags.id',
            'Tags.name',
            'Tags.slug',
        ];
        ...
        if (empty($queryParams) && $this->session->check('Paging.Tags.params')) {
            $savedParams = (array)$this->session->read('Paging.Tags.params');
        ...
        $query = $this->fetchTable('Tags')->find();
        ...
        foreach ($table->associations()->getByType('BelongsTo') as $association) {
            ...
            $query->where(['Tags.' . $foreignKey => $filterValue]);
        ...
        try {
            ...
            if (!empty($queryParams)) {
                $this->session->write('Paging.Tags.params', $queryParams);
            ...
            $this->session->write('Paging.Tags.params', $fallbackParams);
        ...
        $lastViewedId = $this->session->read('LastViewed.tag_id');
        $scrollToId = $this->session->read('ScrollTo.tag_id') ?? $lastViewedId;

        $this->set(compact('tags', 'lastViewedId', 'scrollToId', 'search', 'parentContext'));
    }
    /**
     * View method
     *
    ...
    {
        $tag = $this->Tags->get($id, contain: ['Photos', 'Tags_name_translation', 'I18n']);
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Tags->save($tag)) {
                ...
                $this->session->write('ScrollTo.Admin.tag.id', $tag->id ?? 0);
                return $this->redirect(['action' => 'index']);
            }
    ...
    public function delete($id = null)
    {
        ...
        $this->session->delete('LastViewed.tag_id');
        ...
        if ($neighbor) {
            $this->session->write('ScrollTo.tag_id', (int)$neighbor->id);
        } else {
            $this->session->delete('ScrollTo.tag_id');
        }

        if ($table->delete($tag)) {
            $this->Flash->success(__('The {0} has been successfully deleted.', __('tag')), ['plugin' => 'KvAdmin']);
        } else {
            $this->Flash->error(__('Could not delete the record. Please try again.'), ['plugin' => 'KvAdmin']);
        }

        $redirectParams = (array)$this->session->read('Paging.Tags.params');
        return $this->redirect([
            'action' => 'index',
            '?' => $redirectParams,
        ]);
