...
class ContactMessagesController extends AppController
{
    ...
    public function index()
    {
        ...
        if (isset($queryParams['clear']) && $queryParams['clear'] === 'filter') {
            $this->session->delete('Paging.ContactMessages.params');
        ...
        // =========================================================================
        $searchableFields = [
            // --- 1. Saját tábla (ContactMessages) mezői (sütéskor: string/uuid oszlopok) ---
            // 'ContactMessages.id',
        ...
        if (empty($queryParams) && $this->session->check('Paging.ContactMessages.params')) {
            $savedParams = (array)$this->session->read('Paging.ContactMessages.params');
        ...
        $query = $this->fetchTable('ContactMessages')->find();
        ...
        foreach ($table->associations()->getByType('BelongsTo') as $association) {
            ...
            $query->where(['ContactMessages.' . $foreignKey => $filterValue]);
        ...
        try {
            ...
            if (!empty($queryParams)) {
                $this->session->write('Paging.ContactMessages.params', $queryParams);
            ...
            $this->session->write('Paging.ContactMessages.params', $fallbackParams);
        ...
        $lastViewedId = $this->session->read('LastViewed.contactMessage_id');
        $scrollToId = $this->session->read('ScrollTo.contactMessage_id') ?? $lastViewedId;

        $this->set(compact('contactMessages', 'lastViewedId', 'scrollToId', 'search', 'parentContext'));
    }
    /**
     * View method
     *
    ...
    public function view($id = null)
    {
		...
		$this->session->write('LastViewed.Admin.contactMessage_id', (int)$id ?? 0);
		$this->session->write('ScrollTo.Admin.contactMessage_id', (int)$id ?? 0);
    ...
    public function add()
    {
        $contactMessage = $this->fetchTable('ContactMessages')->newEmptyEntity();
        if ($this->getRequest()->is('post')) {
            $data = $this->getRequest()->getData();
            $contactMessage = $this->fetchTable('ContactMessages')->patchEntity($contactMessage, $data);
            if ($this->fetchTable('ContactMessages')->save($contactMessage)) {
                $this->Flash->success(__('The {0} has been saved.'), __('contact message'), ['plugin' => 'KvAdmin']);

                // Frissen létrehozott rekord megjelölése visszagörgetéshez az index nézetben
                $this->session->write('ScrollTo.Admin.contactMessage.id', $contactMessage->id ?? 0);
                return $this->redirect(['action' => 'index']);
            }
    ...
    {
        $contactMessage = $this->fetchTable('ContactMessages')->get($id, contain: []);
		$this->session->write('LastViewed.Admin.contactMessage_id', (int)$id ?? 0);
		$this->session->write('ScrollTo.Admin.contactMessage_id', (int)$id ?? 0);
        ...
        if ($this->getRequest()->is(['patch', 'post', 'put'])) {
            ...
            if ($this->fetchTable('ContactMessages')->save($contactMessage)) {
                ...
                $redirectParams = (array)$this->session->read('Paging.Admin.ContactMessages.params');
                return $this->redirect([
                    'action' => 'index',
                    '?' => $redirectParams,
                ]);
    ...
    public function delete($id = null)
    {
        ...
        $table = $this->fetchTable('ContactMessages');
        $contactMessage = $table->get($id);
		$contactMessageName = $contactMessage->name;

        $this->session->delete('LastViewed.contactMessage_id');
        ...
        if ($neighbor) {
            $this->session->write('ScrollTo.contactMessage_id', (int)$neighbor->id);
        } else {
            $this->session->delete('ScrollTo.contactMessage_id');
        }

        if ($table->delete($contactMessage)) {
            $this->Flash->success(__('The {0} has been successfully deleted.', __('contact message')), ['plugin' => 'KvAdmin']);
        } else {
            $this->Flash->error(__('Could not delete the record. Please try again.'), ['plugin' => 'KvAdmin']);
        }

        $redirectParams = (array)$this->session->read('Paging.ContactMessages.params');
        return $this->redirect([
            'action' => 'index',
            '?' => $redirectParams,
        ]);
