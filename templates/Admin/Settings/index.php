 ...
 * @var iterable<\App\Model\Entity\> $settings
...
$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.setting_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.setting_id') ?? $lastViewedId;
...
<div class="card">
    ...
    <div class="card-header border-bottom-0 pb-0">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <span class="text-secondary">
                <?= __('{0} rekordhoz tartozó {1}', h($parentContext['label']), __('Settings')) ?>
            </span>
    ...
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új setting gomb -->
            <div class="col-auto">
                <?= $this->KvForm->linkAddNew(['controller' => 'Settings', 'action' => 'add'], __('setting')) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Settings') ?></h3>
...
<?php endif; ?>
            ...
            <tbody>
                ...
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $setting->id); ?>
                <tr
                    id="row-<?= (int)$setting->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $setting->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($setting->id) ?></td>
<?php endif; ?>
                    <td class="string name"><?= $highlight($setting->name) ?></td>
                    <td class="string label"><?= $highlight($setting->label) ?></td>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $setting->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($setting->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($setting->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($setting->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="child-actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->linkChildList('I18n', 'foreign_key', $setting->id, 'Settings', __('I18n')) ?>
                        </div>
                    </td>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $setting->id]) ?>
