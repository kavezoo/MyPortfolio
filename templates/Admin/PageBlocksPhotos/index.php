 ...
 * @var iterable<\App\Model\Entity\> $pageBlocksPhotos
...
$this->assign('title', __('Pageblocksphotos'));
$this->element('KvAdmin.pagination_templates');

$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.pageBlocksPhoto_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.pageBlocksPhoto_id') ?? $lastViewedId;
...
<div class="card">
    ...
    <div class="card-header border-bottom-0 pb-0">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <span class="text-secondary">
                <?= __('{0} rekordhoz tartozó {1}', h($parentContext['label']), __('Page Blocks Photos')) ?>
            </span>
    ...
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új pageBlocksPhoto gomb -->
            <div class="col-auto">
                <?= $this->KvForm->linkAddNew(['controller' => 'Pageblocksphotos', 'action' => 'add'], __('pageBlocksPhoto')) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Pageblocksphotos') ?></h3>
...
<?php endif; ?>
                    <th class="string page_block_id"><?= $this->Paginator->sort('page_block_id') ?></th>
                    <th class="string photo_id"><?= $this->Paginator->sort('photo_id') ?></th>
                    <th class="string css_class"><?= $this->Paginator->sort('css_class') ?></th>
...
<?php endif; ?>
            ...
            <tbody>
                <?php if (!empty($pageBlocksPhotos) && count($pageBlocksPhotos) > 0): ?>
                <?php foreach ($pageBlocksPhotos as $pageBlocksPhoto): ?>
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $pageBlocksPhoto->id); ?>
                <tr
                    id="row-<?= (int)$pageBlocksPhoto->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $pageBlocksPhoto->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($pageBlocksPhoto->id) ?></td>
<?php endif; ?>
                    <td class="string page_block_id">
                        <?= $this->KvForm->linkBelongsToCell($pageBlocksPhoto, 'page_block', 'title', 'PageBlocks', 'id') ?>
                    </td>
                    <td class="string photo_id">
                        <?= $this->KvForm->linkBelongsToCell($pageBlocksPhoto, 'photo', 'title', 'Photos', 'id') ?>
                    </td>
                    <td class="string css_class"><?= $highlight($pageBlocksPhoto->css_class) ?></td>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $pageBlocksPhoto->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($pageBlocksPhoto->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($pageBlocksPhoto->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($pageBlocksPhoto->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $pageBlocksPhoto->id]) ?>
