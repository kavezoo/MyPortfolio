 ...
 * @var iterable<\App\Model\Entity\> $contactMessages
...
$this->assign('title', __('Contactmessages'));
$this->element('KvAdmin.pagination_templates');

$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.contactMessage_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.contactMessage_id') ?? $lastViewedId;
...
<div class="card">
    ...
    <div class="card-header border-bottom-0 pb-0">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <span class="text-secondary">
                <?= __('{0} rekordhoz tartozó {1}', h($parentContext['label']), __('Contact Messages')) ?>
            </span>
    ...
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új contactMessage gomb -->
            <div class="col-auto">
                <?= $this->KvForm->linkAddNew(['controller' => 'Contactmessages', 'action' => 'add'], __('contactMessage')) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Contactmessages') ?></h3>
...
<?php endif; ?>
            ...
            <tbody>
                <?php if (!empty($contactMessages) && count($contactMessages) > 0): ?>
                <?php foreach ($contactMessages as $contactMessage): ?>
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $contactMessage->id); ?>
                <tr
                    id="row-<?= (int)$contactMessage->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $contactMessage->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($contactMessage->id) ?></td>
<?php endif; ?>
                    <td class="string name"><?= $highlight($contactMessage->name) ?></td>
                    <td class="string email"><?= $highlight($contactMessage->email) ?></td>
                    <td class="string phone"><?= $highlight($contactMessage->phone) ?></td>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $contactMessage->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($contactMessage->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($contactMessage->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($contactMessage->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $contactMessage->id]) ?>
                            <?= $this->KvForm->actionEdit(['action' => 'edit', $contactMessage->id]) ?>
                            <?= $this->KvForm->actionDelete(['action' => 'delete', $contactMessage->id], (string)($contactMessage->name ?? '')) ?>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
