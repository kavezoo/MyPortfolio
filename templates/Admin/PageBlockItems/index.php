 ...
 * @var iterable<\App\Model\Entity\> $pageBlockItems
...
$this->assign('title', __('Pageblockitems'));
$this->element('KvAdmin.pagination_templates');

$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.pageBlockItem_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.pageBlockItem_id') ?? $lastViewedId;
...
<div class="card">
    ...
    <div class="card-header border-bottom-0 pb-0">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <span class="text-secondary">
                <?= __('{0} rekordhoz tartozó {1}', h($parentContext['label']), __('Page Block Items')) ?>
            </span>
    ...
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új pageBlockItem gomb -->
            <div class="col-auto">
                <?= $this->KvForm->linkAddNew(['controller' => 'Pageblockitems', 'action' => 'add'], __('pageBlockItem')) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Pageblockitems') ?></h3>
...
<?php endif; ?>
                    ...
                    <th class="string item_type"><?= $this->Paginator->sort('item_type') ?></th>
...
<?php endif; ?>
            ...
            <tbody>
                <?php if (!empty($pageBlockItems) && count($pageBlockItems) > 0): ?>
                <?php foreach ($pageBlockItems as $pageBlockItem): ?>
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $pageBlockItem->id); ?>
                <tr
                    id="row-<?= (int)$pageBlockItem->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $pageBlockItem->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($pageBlockItem->id) ?></td>
<?php endif; ?>
                    <td class="string page_block_id">
                        <?= $this->KvForm->linkBelongsToCell($pageBlockItem, 'page_block', 'title', 'PageBlocks', 'id') ?>
                    </td>
                    <td class="string item_type"><?= $highlight($pageBlockItem->item_type) ?></td>
                    <td class="string url"><?= $highlight($pageBlockItem->url) ?></td>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $pageBlockItem->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($pageBlockItem->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($pageBlockItem->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($pageBlockItem->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="child-actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->linkChildList('I18n', 'foreign_key', $pageBlockItem->id, 'PageBlockItems', __('I18n')) ?>
                        </div>
                    </td>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $pageBlockItem->id]) ?>
