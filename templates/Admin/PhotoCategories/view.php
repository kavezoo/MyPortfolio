...
<div class="page-header d-print-none mb-3 photoCategories">
    <div class="row align-items-center">
        <div class="col">
            <h2 class="page-title"><?= __('{0} megtekintése', __('Photo Category')) ?></h2>
        </div>
...
<div class="card mb-3 photoCategories">
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            <div class="col">
                <h3 class="card-title mb-0"><?= h($photoCategory->name) ?></h3>
            </div>
        ...
        <table class="table table-vcenter card-table table-sm table-bordered-vertical">
            <tbody>
                <tr>
                    ...
                    <td><?= h($photoCategory->name) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Slug') ?></th>
                    <td><?= h($photoCategory->slug) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Photo Categories Name Translation') ?></th>
                    <td><?= $this->KvForm->linkRelatedRecord($photoCategory, 'name_translation', 'locale', 'PhotoCategories_name_translation', 'id') ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Id') ?></th>
                    <td><?= $this->Number->format($photoCategory->id) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Photos Count') ?></th>
                    <td><?= $this->Number->format($photoCategory->photos_count) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Pos') ?></th>
                    <td><?= $this->Number->format($photoCategory->pos) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Created') ?></th>
                    <td><?= h($photoCategory->created) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Modified') ?></th>
                    <td><?= h($photoCategory->modified) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Visible') ?></th>
                    <td><?= $photoCategory->visible ? '<span class=\"badge bg-green-lt\">' . __('Igen') . '</span>' : '<span class=\"badge bg-secondary-lt\">' . __('Nem') . '</span>' ?></td>
                </tr>
            </tbody>
        </table>
    </div>
...
?>
<?php if (!empty($photoCategory->_i18n)) { $hasRelatedRecords = true; } ?>
<?php if (!empty($photoCategory->photos)) { $hasRelatedRecords = true; } ?>
<?php if ($hasRelatedRecords): ?>
<div class="card">
    <div class="card-header">
        <?php $isFirstRelatedTab = true; ?>
        <ul class="nav nav-tabs card-header-tabs" data-bs-toggle="tabs" role="tablist">
            <?php if (!empty($photoCategory->_i18n)): ?>
        ...
        <div class="tab-pane<?= $isFirstRelatedPane ? ' active show' : '' ?>" id="related-_i18n" role="tabpanel">
            <div class="table-responsive">
                <table class="table table-vcenter card-table table-hover table-sm table-bordered-vertical">
                    ...
                    <tbody>
                        <?php foreach ($photoCategory->_i18n as $i18n): ?>
        ...
        <div class="tab-pane<?= $isFirstRelatedPane ? ' active show' : '' ?>" id="related-photos" role="tabpanel">
            <div class="table-responsive">
                <table class="table table-vcenter card-table table-hover table-sm table-bordered-vertical">
                    ...
                    <tbody>
                        <?php foreach ($photoCategory->photos as $photo): ?>
