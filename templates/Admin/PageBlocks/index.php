 ...
 * @var iterable<\App\Model\Entity\> $pageBlocks
...
$this->assign('title', __('Pageblocks'));
$this->element('KvAdmin.pagination_templates');

$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.pageBlock_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.pageBlock_id') ?? $lastViewedId;
...
<div class="card">
    ...
    <div class="card-header border-bottom-0 pb-0">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <span class="text-secondary">
                <?= __('{0} rekordhoz tartozó {1}', h($parentContext['label']), __('Page Blocks')) ?>
            </span>
    ...
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új pageBlock gomb -->
            <div class="col-auto">
                <?= $this->KvForm->linkAddNew(['controller' => 'Pageblocks', 'action' => 'add'], __('pageBlock')) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Pageblocks') ?></h3>
...
<?php endif; ?>
                    <th class="string page_id"><?= $this->Paginator->sort('page_id') ?></th>
                    <th class="string block_type"><?= $this->Paginator->sort('block_type') ?></th>
                    <th class="string layout"><?= $this->Paginator->sort('layout') ?></th>
                    <th class="string title"><?= $this->Paginator->sort('title') ?></th>
                    <th class="string lead"><?= $this->Paginator->sort('lead') ?></th>
                    <th class="string button_label"><?= $this->Paginator->sort('button_label') ?></th>
                    <th class="string button_url"><?= $this->Paginator->sort('button_url') ?></th>
                    <th class="string photo_id"><?= $this->Paginator->sort('photo_id') ?></th>
<?php if (isset($showCounterFields) && $showCounterFields): ?>
                    <th class="integer page_block_items_count"><?= $this->Paginator->sort('page_block_items_count') ?></th>
<?php endif; ?>
<?php if (isset($showCounterFields) && $showCounterFields): ?>
...
<?php endif; ?>
            ...
            <tbody>
                <?php if (!empty($pageBlocks) && count($pageBlocks) > 0): ?>
                <?php foreach ($pageBlocks as $pageBlock): ?>
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $pageBlock->id); ?>
                <tr
                    id="row-<?= (int)$pageBlock->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $pageBlock->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($pageBlock->id) ?></td>
<?php endif; ?>
                    <td class="string page_id">
                        <?= $this->KvForm->linkBelongsToCell($pageBlock, 'page', 'title', 'Pages', 'id') ?>
                    </td>
                    <td class="string block_type"><?= $highlight($pageBlock->block_type) ?></td>
                    <td class="string layout"><?= $highlight($pageBlock->layout) ?></td>
                    <td class="string title"><?= $highlight($pageBlock->title) ?></td>
                    <td class="string lead"><?= $highlight($pageBlock->lead) ?></td>
                    <td class="string button_label"><?= $highlight($pageBlock->button_label) ?></td>
                    <td class="string button_url"><?= $highlight($pageBlock->button_url) ?></td>
                    <td class="string photo_id">
                        <?= $this->KvForm->linkBelongsToCell($pageBlock, 'featured_photo', 'title', 'Photos', 'id') ?>
                    </td>
<?php if (isset($showCounterFields) && $showCounterFields): ?>
                    <td class="integer page_block_items_count text-end"><?= $this->Number->format($pageBlock->page_block_items_count) ?></td>
<?php endif; ?>
<?php if (isset($showCounterFields) && $showCounterFields): ?>
                    <td class="integer photos_count text-end"><?= $this->Number->format($pageBlock->photos_count) ?></td>
<?php endif; ?>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $pageBlock->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($pageBlock->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($pageBlock->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($pageBlock->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="child-actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->linkChildList('I18n', 'foreign_key', $pageBlock->id, 'PageBlocks', __('I18n')) ?>
                            <?= $this->KvForm->linkChildList('PageBlockItems', 'page_block_id', $pageBlock->id, 'PageBlocks', __('PageBlockItems')) ?>
                        </div>
                    </td>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $pageBlock->id]) ?>
