 ...
 * @var iterable<\App\Model\Entity\> $photosTags
...
$this->assign('title', __('Photostags'));
$this->element('KvAdmin.pagination_templates');

$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.photosTag_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.photosTag_id') ?? $lastViewedId;
...
<div class="card">
    ...
    <div class="card-header border-bottom-0 pb-0">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <span class="text-secondary">
                <?= __('{0} rekordhoz tartozó {1}', h($parentContext['label']), __('Photos Tags')) ?>
            </span>
    ...
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új photosTag gomb -->
            <div class="col-auto">
                <?= $this->KvForm->linkAddNew(['controller' => 'Photostags', 'action' => 'add'], __('photosTag')) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Photostags') ?></h3>
...
<?php endif; ?>
                    ...
                    <th class="string tag_id"><?= $this->Paginator->sort('tag_id') ?></th>
...
<?php endif; ?>
            ...
            <tbody>
                <?php if (!empty($photosTags) && count($photosTags) > 0): ?>
                <?php foreach ($photosTags as $photosTag): ?>
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $photosTag->id); ?>
                <tr
                    id="row-<?= (int)$photosTag->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $photosTag->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($photosTag->id) ?></td>
<?php endif; ?>
                    <td class="string photo_id">
                        <?= $this->KvForm->linkBelongsToCell($photosTag, 'photo', 'title', 'Photos', 'id') ?>
                    </td>
                    <td class="string tag_id">
                        <?= $this->KvForm->linkBelongsToCell($photosTag, 'tag', 'name', 'Tags', 'id') ?>
                    </td>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $photosTag->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($photosTag->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($photosTag->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($photosTag->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $photosTag->id]) ?>
