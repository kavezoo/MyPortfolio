 ...
 */
?>
<div class="page-header d-print-none mb-3 blogPosts">
    <div class="row align-items-center">
        <div class="col">
            <h2 class="page-title"><?= __('{0} megtekintése', __('Blog Post')) ?></h2>
        </div>
...
<div class="card mb-3 blogPosts">
    ...
    <div class="table-responsive">
        <table class="table table-vcenter card-table table-sm table-bordered-vertical">
            <tbody>
                ...
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Blog Posts Title Translation') ?></th>
                    <td><?= $this->KvForm->linkRelatedRecord($blogPost, 'title_translation', 'locale', 'BlogPosts_title_translation', 'id') ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Blog Posts Body Translation') ?></th>
                    <td><?= $this->KvForm->linkRelatedRecord($blogPost, 'body_translation', 'locale', 'BlogPosts_body_translation', 'id') ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><?= $this->Number->format($blogPost->photos_count) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Pos') ?></th>
                    <td><?= $this->Number->format($blogPost->pos) ?></td>
                </tr>
                <tr>
                    <th class="w-1 text-nowrap"><?= __('Published') ?></th>
                ...
                <tr>
                    ...
                    <td><?= $blogPost->visible ? '<span class=\"badge bg-green-lt\">' . __('Igen') . '</span>' : '<span class=\"badge bg-secondary-lt\">' . __('Nem') . '</span>' ?></td>
                </tr>
            </tbody>
        </table>
    </div>
    <div class="card-body">
        <div class="mb-3">
            ...
            <div class="text-secondary"><?= $this->Text->autoParagraph(h($blogPost->body)); ?></div>
        </div>
    </div>
</div>
...
?>
<?php if (!empty($blogPost->photos)) { $hasRelatedRecords = true; } ?>
<?php if (!empty($blogPost->_i18n)) { $hasRelatedRecords = true; } ?>
<?php if ($hasRelatedRecords): ?>
<div class="card">
    <div class="card-header">
        <?php $isFirstRelatedTab = true; ?>
        <ul class="nav nav-tabs card-header-tabs" data-bs-toggle="tabs" role="tablist">
            <?php if (!empty($blogPost->photos)): ?>
        ...
        <div class="tab-pane<?= $isFirstRelatedPane ? ' active show' : '' ?>" id="related-photos" role="tabpanel">
            <div class="table-responsive">
                <table class="table table-vcenter card-table table-hover table-sm table-bordered-vertical">
                    ...
                    <tbody>
                        <?php foreach ($blogPost->photos as $photo): ?>
        ...
        <div class="tab-pane<?= $isFirstRelatedPane ? ' active show' : '' ?>" id="related-_i18n" role="tabpanel">
            <div class="table-responsive">
                <table class="table table-vcenter card-table table-hover table-sm table-bordered-vertical">
                    ...
                    <tbody>
                        <?php foreach ($blogPost->_i18n as $i18n): ?>
