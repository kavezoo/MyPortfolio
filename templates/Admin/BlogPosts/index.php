 ...
 * @var iterable<\App\Model\Entity\> $blogPosts
...
$this->assign('title', __('Blogposts'));
$this->element('KvAdmin.pagination_templates');

$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.blogPost_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.blogPost_id') ?? $lastViewedId;
...
<div class="card">
    ...
    <div class="card-header border-bottom-0 pb-0">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <span class="text-secondary">
                <?= __('{0} rekordhoz tartozó {1}', h($parentContext['label']), __('Blog Posts')) ?>
            </span>
    ...
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új blogPost gomb -->
            <div class="col-auto">
                <?= $this->KvForm->linkAddNew(['controller' => 'Blogposts', 'action' => 'add'], __('blogPost')) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Blogposts') ?></h3>
...
<?php endif; ?>
                    ...
                    <th class="datetime published"><?= $this->Paginator->sort('published') ?></th>
...
<?php endif; ?>
            ...
            <tbody>
                <?php if (!empty($blogPosts) && count($blogPosts) > 0): ?>
                <?php foreach ($blogPosts as $blogPost): ?>
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $blogPost->id); ?>
                <tr
                    id="row-<?= (int)$blogPost->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $blogPost->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($blogPost->id) ?></td>
<?php endif; ?>
                    <td class="string slug"><?= $highlight($blogPost->slug) ?></td>
                    <td class="string title"><?= $highlight($blogPost->title) ?></td>
                    <td class="datetime text-nowrap"><?= h($blogPost->published?->format('Y-m-d H:i')) ?></td>
<?php if (isset($showCounterFields) && $showCounterFields): ?>
                    <td class="integer photos_count text-end"><?= $this->Number->format($blogPost->photos_count) ?></td>
<?php endif; ?>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $blogPost->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($blogPost->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($blogPost->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($blogPost->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="child-actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->linkChildList('I18n', 'foreign_key', $blogPost->id, 'BlogPosts', __('I18n')) ?>
                        </div>
                    </td>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $blogPost->id]) ?>
                            <?= $this->KvForm->actionEdit(['action' => 'edit', $blogPost->id]) ?>
                            <?= $this->KvForm->actionDelete(['action' => 'delete', $blogPost->id], (string)($blogPost->title ?? '')) ?>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
