 ...
 * @var iterable<\App\Model\Entity\> $blogPostsPhotos
...
$this->assign('title', __('Blogpostsphotos'));
$this->element('KvAdmin.pagination_templates');

$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.blogPostsPhoto_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.blogPostsPhoto_id') ?? $lastViewedId;
...
<div class="card">
    ...
    <div class="card-header border-bottom-0 pb-0">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <span class="text-secondary">
                <?= __('{0} rekordhoz tartozó {1}', h($parentContext['label']), __('Blog Posts Photos')) ?>
            </span>
    ...
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új blogPostsPhoto gomb -->
            <div class="col-auto">
                <?= $this->KvForm->linkAddNew(['controller' => 'Blogpostsphotos', 'action' => 'add'], __('blogPostsPhoto')) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Blogpostsphotos') ?></h3>
...
<?php endif; ?>
                    <th class="string blog_post_id"><?= $this->Paginator->sort('blog_post_id') ?></th>
...
<?php endif; ?>
            ...
            <tbody>
                <?php if (!empty($blogPostsPhotos) && count($blogPostsPhotos) > 0): ?>
                <?php foreach ($blogPostsPhotos as $blogPostsPhoto): ?>
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $blogPostsPhoto->id); ?>
                <tr
                    id="row-<?= (int)$blogPostsPhoto->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $blogPostsPhoto->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($blogPostsPhoto->id) ?></td>
<?php endif; ?>
                    <td class="string blog_post_id">
                        <?= $this->KvForm->linkBelongsToCell($blogPostsPhoto, 'blog_post', 'title', 'BlogPosts', 'id') ?>
                    </td>
                    <td class="string photo_id">
                        <?= $this->KvForm->linkBelongsToCell($blogPostsPhoto, 'photo', 'title', 'Photos', 'id') ?>
                    </td>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $blogPostsPhoto->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($blogPostsPhoto->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($blogPostsPhoto->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($blogPostsPhoto->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $blogPostsPhoto->id]) ?>
